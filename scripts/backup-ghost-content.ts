...
if (!process.env.GHOST_URL || !process.env.GHOST_CONTENT_API_KEY) {
  throw new Error('GHOST_URL and GHOST_CONTENT_API_KEY must be set in environment variables')
}
...
async function backupGhostContent() {
  try {
    ...
    const backupDir = path.join(process.cwd(), 'src', 'content', 'blog', 'backups', date)
